...
class ShopController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        ...
        try {
            $order = DB::transaction(function () use ($cart, $user, $affiliateId) {
                ...
                $order = Order::create([
                    ...
                    'notes' => 'Created from affiliate cart checkout',
                ]);
        ...
        try {
            ...
            Log::error('Midtrans transaction create failed: ' . $e->getMessage(), [
                'order_id' => $order->id,
            ]);
        ...
        // Clear cart after order created and stock reserved.
    ...
    public function cancel(Request $request)
    {
        ...
        if ($pendingOrder) {
            app(OrderService::class)->cancelPendingOrder(
                $pendingOrder->id,
                'Cancelled by user from affiliate cart'
            );
        }
    ...
    private function createMidtransTransaction(Order $order, $user): string
    {
        ...
        $params = [
            ...
            'callbacks' => [
                'finish' => route('affiliate.shop.index'),
                'error' => route('affiliate.shop.index'),
                'pending' => route('affiliate.shop.index'),
            ],
        ];
